...
fh = logging.FileHandler("roberta_exp2_new.log", encoding="utf-8")
# 把输出处理器添加到日志器中
...
df = pd.read_json('/root/new_exp/XL/以transformer-xl作为提取模型/chatGPT/data_all_eTz/law_eTz_all.json')
df_texts = []
df_labels = []
...
test_all_t = []
test_l = []
df_test = pd.read_csv('/root/new_exp/XL/以transformer-xl作为提取模型/chatGPT/data_chatgpt_open/cnews_2000/chatgpt_cnews_prefix.csv')
...
logger.info(train_input_ids.shape)
############################处理验证集########################################
valid_input_ids = []
...
logger.info(valid_input_ids.shape)
############################处理测试集########################################
test_input_ids = []
...
logger.info(test_input_ids.shape)
#数据集的装载
