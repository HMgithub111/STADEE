...
fh = logging.FileHandler("roberta_exp3_new.log", encoding="utf-8")
# 把输出处理器添加到日志器中
...
df = pd.read_csv('/root/new_exp/XL/以transformer-xl作为提取模型/用cpm低p生成一些文本/cnews.csv')
df_texts = []
df_labels = []
...
val_all_t = [df_texts[i] for i in random_idx[7*one_train_length:]]
val_l = [df_labels[i] for i in random_idx[7*one_train_length:]]

test_all_t = []
test_l = []
df_test = pd.read_json('/root/new_exp/XL/以transformer-xl作为提取模型/chatGPT/data_all_eTz/finance_eTz_all.json')
test_file = ['medicine_eTz_all','psychology_eTz_all','baike_eTz_all','law_eTz_all','nlpcc_eTz_all','openqa_eTz_all']
for file in test_file:
    df_test = pd.concat([df_test,pd.read_json('/root/new_exp/XL/以transformer-xl作为提取模型/chatGPT/data_all_eTz/'+file+'.json')])
for i in range(len(df_test)):
    df_test.iloc[i,0] = df_test.iloc[i,0].replace(',','，')
    df_test.iloc[i,0] = df_test.iloc[i,0].replace('\\n','')
    df_test.iloc[i,0] = df_test.iloc[i,0].replace('\n','')
    if len(df_test.iloc[i,0])>10:
        test_all_t.append(df_test.iloc[i,0])
        test_l.append(df_test.iloc[i,1])
#########################################
